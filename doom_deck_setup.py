...
GITHUB_USER_AGENT = f"doom-deck-setup/{SCRIPT_VERSION}"
...
def write_wrappers(dirs: Dirs, dry_run: bool, logger: logging.Logger) -> None:
    ...
    common_shell = """# This wrapper is generated by doom_deck_setup.py.
...
def build_uzdoom_ini() -> str:
    lines = ["; Generated by doom_deck_setup.py", "[GlobalSettings]"]
...
def write_uzdoom_configs(dirs: Dirs, dry_run: bool, logger: logging.Logger) -> None:
    ...
    classic_cfg = f"""// Generated by doom_deck_setup.py
...
bind 7 slot 7
"""
    modern_cfg = f"""// Generated by doom_deck_setup.py
...
def generate_preset_manifest(dirs: Dirs, brutal_path: Optional[Path], project_brutality_path: Optional[Path]) -> dict[str, Any]:
    ...
    return {
        ...
        "generated_at": _dt.datetime.now().isoformat(timespec="seconds"),
        "warning": "This is a stable manifest generated by this script. Doom Runner's live options.json is generated separately.",
        "root": str(dirs.root),
        "engine": {
            "name": "UZDoom",
...
Generated by `doom_deck_setup.py`.
...
This script installs Doom Runner and UZDoom, copies Steam IWADs, creates direct shell launchers, writes Doom Runner's live options file, and writes a stable preset manifest:
...
The default update channel is `latest`, which can include beta/test builds such as v22 test releases. To prefer the newest non-beta candidate the script can identify, use:
...
```bash
python3 doom_deck_setup.py install --brutal-doom-file /path/to/brutal-doom.pk3
```

To skip Brutal Doom update checks:

```bash
python3 doom_deck_setup.py install --skip-brutal-doom
```
...
```bash
python3 doom_deck_setup.py install --project-brutality-file /path/to/Project_Brutality.pk3
```
